...
def Q_1(conn, execution_time):
    ...
    query = """
            SELECT player_name, avg_score
            FROM Player INNER JOIN (
                SELECT player, avg(statsbomb_xg) AS avg_score
                FROM (
                    SELECT ShotEvent.event_id, player, statsbomb_xg
                    FROM ShotEvent INNER JOIN (	
                        ...
                        FROM Event INNER JOIN (
                            ...
                            FROM Match INNER JOIN (
                                ...
                                FROM (
                                    SELECT Competition.season_id, season_name, competition_id, competition_name
                                    FROM Competition 
                                    INNER JOIN Season 
                            ...
                            ON Match.competition = sc.competition_id AND Match.season = sc.season_id
                        ) Teams
                        ON Event.possession_team = Teams.home_team 
                        OR Event.possession_team = Teams.away_team 
                        OR Event.team = Teams.home_team 
                        OR Event.team = Teams.away_team
                    ) relevant_events
                    ON ShotEvent.event_id = relevant_events.event_id
                )
                GROUP BY player
                HAVING avg(statsbomb_xg) > 0
            ) player_averages
            ON Player.player_id = player_averages.player
...
def Q_2(conn, execution_time):
    ...
    query = """
            SELECT player_name, num_shots
            FROM Player INNER JOIN (
                SELECT player, count(*) AS num_shots
                FROM (
                    SELECT ShotEvent.event_id, player
                    FROM ShotEvent INNER JOIN (	
                        SELECT DISTINCT event_id
                        FROM Event INNER JOIN (
                            SELECT DISTINCT home_team, away_team
            ...
            ON Player.player_id = player_shot_counts.player
